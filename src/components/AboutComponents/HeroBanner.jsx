...
export default function HeroBanner() {
  ...
  // Stats data
  const stats = [
    { number: counts.years + "+", text: "Years Experience", color: "from-purple-400 to-purple-600" },
    { number: counts.projects + "+", text: "Projects Done", color: "from-blue-400 to-blue-600" },
    { number: counts.clients + "+", text: "Happy Clients", color: "from-pink-400 to-pink-600" },
  ];

  return (
    <section className="relative py-16 px-6 md:px-12 max-w-screen-xl mx-auto h-screen overflow-hidden">
      {/* Decorative background elements */}
      <div className="absolute top-0 right-0 w-64 h-64 bg-purple-300 rounded-full opacity-10 blur-3xl"></div>
      <div className="absolute bottom-0 left-0 w-80 h-80 bg-blue-300 rounded-full opacity-10 blur-3xl"></div>
      
      {/* Animated particles */}
      <div className="absolute top-20 left-20 w-6 h-6 bg-purple-500 rounded-full opacity-20 animate-pulse"></div>
      <div className="absolute top-40 right-40 w-4 h-4 bg-blue-500 rounded-full opacity-20 animate-ping"></div>
      <div className="absolute bottom-32 left-1/2 w-5 h-5 bg-pink-500 rounded-full opacity-20 animate-bounce"></div>
      
      <div className="grid grid-cols-1 lg:grid-cols-2 items-center gap-12 h-full">
        {/* Text Content */}
        <div className="text-center lg:text-left">
          <div 
            className={`overflow-hidden transition-all duration-1000 ${isVisible ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform -translate-y-10'}`}
          >
          ...
          <div 
            className={`transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform translate-y-10'}`}
          >
            <p className="text-base sm:text-lg md:text-xl text-gray-600 mt-6 leading-relaxed max-w-lg">
              Transforming ideas into elegant solutions through creative design and innovative development that captivates and engages your audience.
            </p>
          </div>

          <div 
            className={`mt-10 transition-all duration-1000 delay-500 ${isVisible ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform translate-y-10'}`}
          >
              ...
              {stats.map((item, index) => (
                ...
                >
                  ...
                  <div className="relative overflow-hidden p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg transform transition-all duration-300 hover:scale-105 hover:shadow-xl z-10">
                    <div className={`absolute bottom-0 left-0 h-1 w-full bg-gradient-to-r ${item.color} transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left`}></div>
        ...
        {/* Image */}
        <div className={`flex justify-center items-center transition-all duration-1000 delay-700 ${isVisible ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform translate-y-10'}`}>
